...
こんにちはXEMBookです。この章ではブラウザを活用してNEMの可能性について触れみたいと思います。ぜひ最後までお付き合いください。

## まず動かしてみたいひとへ
人が新しい技術に触れる時、そのきっかけはいろいろあります。でも、まず仕様書を見て感動したから、という人はあまりいません。NEMに使用されている技術は感動するに値する価値のあるものですが、たいていは「自分の信頼する誰かが褒めていたから」だったり「いつも便利に使ってるものがその技術で作られていたから」だったりするものです。新しい技術を採用する立場にある人も同様です。同時に多くの技術を比較しなければいけないため、その技術習得のために新たなプラットフォームを身につけないといけないということであればなおさらハードルは上がってしまいます。ブロックチェーンは社会実装という言葉も使われているとおり、IT技術とは少し離れた専門家の人にも注目してほしい技術です。しかしながら、さまざまなブロックチェーン技術はノード構築の知識を必要とするものが多くNEMのようなAPIでそのため、本章のサンプルプログラムはできるだけシンプルにNEMの可能性を感じられるように注力しました。本章を読んだみなさんが、サンプルプログラムを動かして、足りないと思ったところや興味の出てきたところを自分で調べられるようになれれば幸いです。

## 解説の流れ
まず、開発環境の準備とデバッグ方法について説明した後、トランザクション生成の基礎編と応用編についてご紹介します。最後に、実社会に導入する場合にどういった点に注意すべきかとNEMならではの解決方法を提示しながらご紹介できればと思います。
